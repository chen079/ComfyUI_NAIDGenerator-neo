...
def encode_vibe(access_token, image, model, information_extracted, timeout=120):
    data = { "image": "image", "model": model, "information_extracted": information_extracted }
    files = {
        "image": ("blob", base64.b64decode(image), "image/png"),
        "request": ("blob", json.dumps(data), "application/json"),
    }
    response = requests.post(f"{BASE_URL}/ai/encode-vibe", files=files, headers={ "Authorization": f"Bearer {access_token}" }, timeout=timeout)
    try:
        response.raise_for_status()
    except requests.HTTPError as error:
        detail = response.text.replace(access_token, "[redacted]").replace(image, "[image]").strip()[:500]
        correlation_id = response.headers.get("x-correlation-id", "unavailable")
        error.args = (f"{error}\nNovelAI encode-vibe response: {detail or '(empty body)'}\nCorrelation ID: {correlation_id}",)
        raise
