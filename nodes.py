...
class EncodeVibe:
    ...
    def INPUT_TYPES(s):
        return { "required": {
            "image": ("IMAGE",),
            "model": (["nai-diffusion-4-curated-preview", "nai-diffusion-4-full", "nai-diffusion-4-5-curated", "nai-diffusion-4-5-full"], { "default": "nai-diffusion-4-5-full" }),
            "information_extracted": ("FLOAT", { "default": 1.0, "min": 0.01, "max": 1.0, "step": 0.01, "display": "number", "tooltip": "NovelAI charges for each encoding. Changing this value requires a new encoding." }),
            "timeout_sec": ("INT", { "default": 120, "min": 30, "max": 3000, "step": 1 }),
        }}

    RETURN_TYPES = ("NAID_VIBE",)
    ...
    def encode(self, image, model, information_extracted, timeout_sec):
        if image.shape[0] != 1:
            raise ValueError("EncodeVibe accepts one image. Use a separate encoder for each reference.")
        encoding = encode_vibe(get_access_token(), image_to_base64(image), model, information_extracted, timeout_sec)
...
class VibeTransferOption:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "information_extracted": ("FLOAT", { "default": 1.0, "min": 0.01, "max": 1.0, "step": 0.01, "display": "number", "tooltip": "For raw V3 images only. With encoded_vibe, set this on EncodeVibe instead." }),
                "strength": ("FLOAT", { "default": 0.6, "min": 0.01, "max": 1.0, "step": 0.01, "display": "number" }),
            },
            "optional": { "image": ("IMAGE",), "option": ("NAID_OPTION",), "encoded_vibe": ("NAID_VIBE",) },
        }
    RETURN_TYPES = ("NAID_OPTION",)
    FUNCTION = "set_option"
    CATEGORY = "NovelAI"
    def set_option(self, image=None, information_extracted=1.0, strength=0.6, option=None, encoded_vibe=None):
        option = copy.deepcopy(option) if option else {}
        if encoded_vibe is not None:
            if image is not None:
                raise ValueError("Connect either image or encoded_vibe to VibeTransferOption, not both.")
            if option.get("model", encoded_vibe["model"]) != encoded_vibe["model"]:
                raise ValueError("The vibe encoding model must match the generation model.")
            option["model"] = encoded_vibe["model"]
            option.setdefault("encoded_vibe", []).append((encoded_vibe, strength))
            return (option,)
        if image is None:
            raise ValueError("Connect an image or an encoded_vibe to VibeTransferOption.")
...
class GenerateNAID:
    ...
    def generate(self, limit_opus_free, width, height, positive, negative, steps, cfg, decrisper, variety, smea, sampler, scheduler, seed, uncond_scale, cfg_rescale, keep_alpha, option=None):
        ...
        if option:
            ...
            if "vibe" in option:
                if option["vibe"] and model.startswith("nai-diffusion-4"):
            ...
            if option.get("encoded_vibe"):
                for vibe, strength in option["encoded_vibe"]:
                    if vibe["model"] != model:
                        raise ValueError("The vibe encoding model must match the generation model.")
